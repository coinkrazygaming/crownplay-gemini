
import React, { useState, useRef } from 'react';
import { Layout, Card, Button, Input, Badge, BadgeIcon } from '../components';
import { useStore } from '../store';
import { KYCStatus, KYCDocuments } from '../types';
import { REFERRAL_BONUS_GC, REFERRAL_BONUS_SC } from '../constants';

const ProfilePage: React.FC = () => {
  const { currentUser, users, updateUser, settings, connectSocial, completeSocialPromotion, uploadKycDoc } = useStore();
  const [formData, setFormData] = useState({
    name: currentUser?.name || '',
    dob: currentUser?.dob || '',
    address: currentUser?.address || ''
  });

  const fileInputRef = useRef<HTMLInputElement>(null);
  const [uploadType, setUploadType] = useState<keyof KYCDocuments | null>(null);

  if (!currentUser) return null;

  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file && uploadType) {
      const reader = new FileReader();
      reader.onload = (event) => {
        uploadKycDoc(uploadType, event.target?.result as string);
        alert(`${uploadType.toUpperCase()} Uploaded to Identity Vault.`);
      };
      reader.readAsDataURL(file);
    }
  };

  const triggerUpload = (type: keyof KYCDocuments) => {
    setUploadType(type);
    fileInputRef.current?.click();
  };

  const referredCitizens = users.filter(u => u.referredBy === currentUser.id);

  return (
    <Layout>
      <div className="max-w-6xl mx-auto space-y-12">
        <header className="flex items-center justify-between border-b border-zinc-900 pb-10">
           <div className="flex items-center gap-6">
             <div className="w-24 h-24 rounded-3xl bg-zinc-900 border-2 border-amber-500/50 flex flex-col items-center justify-center shadow-xl">
                <span className="text-3xl font-black italic text-amber-500 leading-none">{currentUser.level}</span>
                <span className="text-[8px] font-black uppercase text-zinc-500 tracking-widest mt-1">LVL</span>
             </div>
             <div>
               <h1 className="text-5xl font-black italic tracking-tighter uppercase mb-2">My Sovereignty</h1>
               <div className="flex gap-2">
                  <Badge variant={currentUser.kycStatus === 'VERIFIED' ? 'success' : 'warning'}>{currentUser.kycStatus}</Badge>
                  {currentUser.badges.map(b => <BadgeIcon key={b} type={b} />)}
               </div>
             </div>
           </div>
        </header>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-12">
            
            {/* KYC Identity Vault */}
            <Card className="p-10 border-amber-500/20 bg-amber-500/5">
               <h3 className="text-2xl font-black uppercase italic mb-8 flex items-center gap-3">
                  <span className="text-3xl">🛡️</span> Identity Vault
               </h3>
               <p className="text-sm text-zinc-400 mb-10 leading-relaxed border-l-2 border-amber-500/30 pl-6">
                 To maintain a secure kingdom and process redemptions, you must verify your identity. All data is end-to-end encrypted in the monarch archives.
               </p>
               
               <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} accept="image/*" />
               
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <KycBox 
                    label="Government ID Front" 
                    uploaded={!!currentUser.kycDocuments.idFront} 
                    onClick={() => triggerUpload('idFront')} 
                  />
                  <KycBox 
                    label="Proof of Address" 
                    uploaded={!!currentUser.kycDocuments.proofOfAddress} 
                    onClick={() => triggerUpload('proofOfAddress')} 
                  />
                  <KycBox 
                    label="Payment Card Verification" 
                    uploaded={!!currentUser.kycDocuments.paymentProof} 
                    onClick={() => triggerUpload('paymentProof')} 
                  />
               </div>
               
               {currentUser.kycStatus === 'PENDING' && (
                 <div className="mt-8 p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-center">
                    <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest animate-pulse">Your documents are under review by the High Auditor.</p>
                 </div>
               )}
            </Card>

            <Card className="p-10">
               <h3 className="text-2xl font-black mb-8 uppercase italic">Sovereign Profile</h3>
               <form className="space-y-8" onSubmit={(e) => { e.preventDefault(); updateUser(formData); alert("Vault Synchronized."); }}>
                 <Input label="Full Legal Name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <Input label="Date of Birth" type="date" value={formData.dob} onChange={e => setFormData({...formData, dob: e.target.value})} />
                   <Input label="Residential Address" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                 </div>
                 <div className="pt-6 border-t border-zinc-800 flex justify-end">
                   <Button variant="primary" className="px-12 py-4">UPDATE IDENTITY</Button>
                 </div>
               </form>
            </Card>
          </div>

          <div className="space-y-8">
            <Card className="p-8 bg-zinc-900/50">
               <h3 className="text-lg font-black mb-6 uppercase italic tracking-tighter">Citizen Stats</h3>
               <div className="space-y-4 text-xs font-bold uppercase tracking-widest">
                  <div className="flex justify-between items-center p-3 bg-zinc-950 rounded-xl border border-zinc-800">
                     <span className="text-zinc-500">Citizen ID</span>
                     <span className="text-white font-mono">CP-{currentUser.id.toUpperCase()}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-zinc-950 rounded-xl border border-zinc-800">
                     <span className="text-zinc-500">Ambassador Count</span>
                     <span className="text-amber-500">{referredCitizens.length}</span>
                  </div>
               </div>
            </Card>

            <Card className="p-8">
               <h3 className="text-lg font-black mb-6 uppercase italic tracking-tighter">Sovereign Badges</h3>
               <div className="flex flex-col gap-3">
                  {currentUser.badges.map(b => <BadgeIcon key={b} type={b} />)}
                  {currentUser.badges.length === 0 && <p className="text-[10px] text-zinc-600 italic uppercase">No royal honors yet.</p>}
               </div>
            </Card>
          </div>
        </div>
      </div>
    </Layout>
  );
};

const KycBox = ({ label, uploaded, onClick }: any) => (
  <button 
    onClick={onClick}
    className={`p-6 rounded-[32px] border-2 flex flex-col items-center justify-center text-center transition-all ${uploaded ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-500' : 'bg-zinc-950 border-dashed border-zinc-800 text-zinc-600 hover:border-amber-500/50 hover:bg-zinc-900'}`}
  >
     <div className="text-3xl mb-4">{uploaded ? '✅' : '📤'}</div>
     <div className="text-[9px] font-black uppercase tracking-widest">{label}</div>
     <div className="text-[8px] mt-2 opacity-50 uppercase">{uploaded ? 'VAULT SECURED' : 'TAP TO UPLOAD'}</div>
  </button>
);

export default ProfilePage;
